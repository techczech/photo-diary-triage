...
final class AppState: ObservableObject {
    ...
    var browserRoots: [BrowserNode] {
        ...
        if let session = currentSession {
            roots.append(
                BrowserNode(
                    ...
                    parentID: nil,
                    mediaItemIDs: [],
                    children: [buildSessionRoot(for: session)],
                    folderURL: nil
                )
            )
        } else {
        ...
        roots.append(buildArchiveSection())
        return roots
    }
    ...
    func openSession(for folder: URL) async {
        do {
            ...
            selectedSidebarNodeID = preferredInitialSidebarNodeID()
    ...
    private func regroupCurrentSession(statusPrefix: String) {
        ...
        let fallbackSidebarNodeID = preferredInitialSidebarNodeID(for: session, bursts: grouped.burstGroups, clusters: grouped.timeClusters)
    ...
    private func loadMostRecentSession() {
        ...
        selectedSidebarNodeID = currentSession == nil ? "section-current-session" : preferredInitialSidebarNodeID()
        clearDetailSelections()
        archiveMediaCache.removeAll()
        resetInlineExpansionState()
    ...
    private func buildSessionRoot(for session: ImportSession) -> BrowserNode {
        buildSessionRoot(for: session, bursts: burstGroups, clusters: timeClusters)
    }
    ...
    private func buildArchiveSection() -> BrowserNode {
        let rootURL = settings.archiveRoot
    ...
    private func loadArchiveMediaIfNeeded(for nodeID: String?) {
        guard let nodeID, archiveMediaCache[nodeID] == nil, let node = browserNodeMap[nodeID] else { return }
        guard (node.children?.isEmpty ?? true), let folderURL = node.folderURL, node.kind == .archiveWalkFolder else { return }

        do {
            let items = try scanner.scanFolder(folderURL, settings: settings)
            archiveMediaCache[nodeID] = items
            requestThumbnails(for: items)
    ...
    private func preferredInitialSidebarNodeID() -> String {
        preferredInitialSidebarNodeID(for: currentSession, bursts: burstGroups, clusters: timeClusters)
    }

    private func preferredInitialSidebarNodeID(for session: ImportSession?, bursts: [BurstGroup], clusters: [TimeCluster]) -> String {
        ...
        var nodeMap: [String: BrowserNode] = [:]
