...
@Test func importCoordinatorCommitCopiesSelectedFilesAndWritesManifests() async throws {
    ...
    #expect(result.fileManifests[0].thumbnailCacheKey == importedItem.thumbnailCacheKey)
    ...
    let fileManifestText = try String(contentsOf: fileManifestURL, encoding: .utf8)
    #expect(fileManifestText.contains("thumbnail_cache_key:"))
